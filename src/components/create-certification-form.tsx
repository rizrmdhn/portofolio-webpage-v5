...
export default function CreateCertificationForm() {
  const utils = api.useUtils();
  const router = useRouter();

  const { mutate, status } = api.certification.create.useMutation({
    onSuccess: () => {
      globalSuccessToast("Certification created successfully");
      utils.certification.getAll.invalidate();
      router.back();
    },
    onError: (error) => {
      globalErrorToast(error.message || "Failed to create certification");
    },
  });

  const form = useForm<
    z.infer<typeof certificationSchema.createCertificationSchema>
  >({
    resolver: zodResolver(certificationSchema.createCertificationSchema),
    defaultValues: {},
  });
        ...
        >
          <FieldGroup>
            ...
            <Button type="submit" disabled={status === "pending"}>
              ...
              {status === "pending" ? "Creating..." : "Create Certification"}
            </Button>
          </FieldGroup>
        </form>
      </div>
