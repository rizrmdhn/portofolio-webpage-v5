...
export default function CreateExperienceForm() {
	...
	const { mutate, status } = api.experience.create.useMutation({
		onSuccess: () => {
		...
		onError: (error) => {
			globalErrorToast(error.message || "Failed to create experience");
		},
	});

	const form = useForm<z.infer<typeof experienceSchema.createExperienceSchema>>(
		{
			resolver: zodResolver(experienceSchema.createExperienceSchema),
			defaultValues: {
				title: "",
				company: "",
				description: "",
				...
				>
					<FieldGroup>
						...
						<Button type="submit" disabled={status === "pending"}>
							...
							{status === "pending" ? "Creating..." : "Create Experience"}
						</Button>
					</FieldGroup>
				</form>
			</div>
